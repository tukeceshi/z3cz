...
export function AiGenerateButton({
  ...
  return (
    <button
      type="button"
      disabled={isCancelling || (disabled && !(isGenerating && canCancel))}
      aria-label={
        isCancelling ? label : showStop ? (cancelLabel ?? label) : label
      }
      title={isCancelling ? label : showStop ? (cancelLabel ?? label) : label}
      className={cn(
