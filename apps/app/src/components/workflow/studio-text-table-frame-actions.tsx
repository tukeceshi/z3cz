...
export interface StudioTextTableFrameActionsProps {
  readonly tableMarkdown: string;
  readonly precedingText: string;
  readonly className?: string;
}

export function StudioTextTableFrameActions({
  tableMarkdown,
  precedingText,
  className,
}: StudioTextTableFrameActionsProps) {
  ...
  const handleSelect = useCallback(
    (nodeType: AiGenerativeNodeType) => {
      studio?.addGenerativeNode?.(nodeType, {
        prompt: tableMarkdown,
        precedingText,
      });
      setOpen(false);
    },
    [precedingText, studio, tableMarkdown]
  );

  if (!studio?.addGenerativeNode) {
    return null;
  }
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              "studio-text-table-frame-action pointer-events-auto",
            ...
            aria-label={t("workflow.studio.tableCreateNodeAria")}
          >
            <Plus className="size-4 shrink-0" aria-hidden />
            {t("workflow.studio.tableCreateNode")}
          </button>
        </PopoverTrigger>
        <PopoverContent
          align="center"
