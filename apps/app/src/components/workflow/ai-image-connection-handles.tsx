...
const AI_IMAGE_HANDLE_RF_Y_OFFSET_PX = AI_TEXT_HANDLE_HIT_PX / 2;

const AI_IMAGE_PLUS_Y_MAX_PX =
...
function clampPlusYOffset(y: number): number {
  return Math.max(-AI_IMAGE_PLUS_Y_MAX_PX, Math.min(AI_IMAGE_PLUS_Y_MAX_PX, y));
}

function AiImageHandlePlusIcon() {
  return (
    <svg
...
interface AiImageEdgeSideProps {
  readonly side: "left" | "right";
  readonly disabled?: boolean;
}

function AiImageEdgeSide({ side, disabled = false }: AiImageEdgeSideProps) {
  ...
  const handleId = isSource ? AI_IMAGE_OUTPUT_ID : AI_IMAGE_REFERENCE_HANDLE_ID;
  ...
  const handleStyle =
    side === "left"
      ? {
          left: AI_TEXT_EDGE_PLUS_OUTER_PX,
          top: AI_IMAGE_HANDLE_RF_Y_OFFSET_PX,
...
export function AiImageConnectionSides({
  ...
  return (
    <>
      <AiImageEdgeSide side="left" disabled={disabled} />
      <AiImageEdgeSide side="right" disabled={disabled} />
    </>
  );
}
