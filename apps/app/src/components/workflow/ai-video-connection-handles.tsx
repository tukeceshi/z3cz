...
const AI_VIDEO_HANDLE_RF_Y_OFFSET_PX = AI_TEXT_HANDLE_HIT_PX / 2;

const AI_VIDEO_PLUS_Y_MAX_PX =
...
function clampPlusYOffset(y: number): number {
  return Math.max(-AI_VIDEO_PLUS_Y_MAX_PX, Math.min(AI_VIDEO_PLUS_Y_MAX_PX, y));
}

function AiVideoHandlePlusIcon() {
  return (
    <svg
...
interface AiVideoEdgeSideProps {
  readonly side: "left" | "right";
  readonly disabled?: boolean;
}

function AiVideoEdgeSide({ side, disabled = false }: AiVideoEdgeSideProps) {
  ...
  const handleId = isSource ? AI_VIDEO_OUTPUT_ID : AI_VIDEO_REFERENCE_HANDLE_ID;
  ...
  const handleStyle =
    side === "left"
      ? {
          left: AI_TEXT_EDGE_PLUS_OUTER_PX,
          top: AI_VIDEO_HANDLE_RF_Y_OFFSET_PX,
...
export function AiVideoConnectionSides({
  ...
  return (
    <>
      <AiVideoEdgeSide side="left" disabled={disabled} />
      <AiVideoEdgeSide side="right" disabled={disabled} />
    </>
  );
}
