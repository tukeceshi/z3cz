...
export const STUDIO_META_TAG = `inline-flex max-w-full shrink-0 items-center rounded-md border ${SURFACE_BORDER_SUBTLE} px-1.5 py-0.5 text-[11px] text-muted-foreground`;
...
export const STUDIO_TAB = `inline-flex h-8 shrink-0 items-center rounded-md border ${SURFACE_BORDER_SUBTLE} px-3 text-sm font-medium leading-none text-muted-foreground transition-colors ${SURFACE_CONTROL_HOVER} hover:text-foreground`;

export const STUDIO_TAB_ACTIVE = `border ${SURFACE_BORDER_SUBTLE} ${SURFACE_CONTROL_ACTIVE}`;
...
export const STUDIO_AUDIO_TILE_SQUARE = `relative aspect-square overflow-hidden rounded-lg ${SURFACE_MUTED_FILL} transition-colors`;
