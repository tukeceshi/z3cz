...
import {
  canGenerativeCardUpload,
  hasGenerativePrompt,
} from "./generative-card-upload-utils";

interface UseGenerativeCardUploadParams {
...
export function useGenerativeCardUpload({
  prompt,
  hasMedia,
  isGenerating,
  disabled = false,
  ...
  onClearPrompt,
  i18nPrefix,
}: UseGenerativeCardUploadParams) {
  const { t } = useTranslation();
  const toast = useAppToast();
  const [confirmOpen, setConfirmOpen] = useState(false);

  const canUpload = canGenerativeCardUpload({
    hasMedia,
    isGenerating,
