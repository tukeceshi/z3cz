...
function WizardStep2CardSkeletons({ count }: { readonly count: number }) {
  return (
...
function activationByCanonicalId(
  ...
  return Object.fromEntries(
    results.map((result) => [result.canonicalId, result])
  );
}
...
function emptyEnabledModels(
  ...
  return Object.fromEntries(catalog.map((entry) => [entry.canonicalId, false]));
}
...
function enabledModelsFromProbeResults(
  ...
  return Object.fromEntries(
    catalog.map((entry) => {
      const probe = results.find(
        (result) => result.canonicalId === entry.canonicalId
      );
...
export function VolcanoWizardFlow({
  ...
  const { catalog: aggregateCatalog, catalogError } =
  ...
  const runTosProbe =
    useCallback(async (): Promise<VolcanoTosServiceStatus> => {
  ...
  const runStep2Probe = useCallback(async () => {
    ...
    try {
      ...
      if (normalizedResults.some((result) => result.status === "auth_error")) {
        setProbeError(
          t("pages.aiInterfaces.volcano.activation.authGlobalError")
        );
      ...
      if (error instanceof ApiRequestError && error.status === 401) {
        setProbeError(
          t("pages.aiInterfaces.volcano.activation.sessionExpired")
        );
  ...
  const handleSave = async () => {
    ...
    try {
      const includeTos = tosConfig.enabled && tosServiceStatus !== "not_opened";
            ...
            {probePhase === "loading" ? (
              <div className="space-y-3">
                ...
                <WizardStep2CardSkeletons
                  count={aggregateCatalog.length || 8}
                />
              </div>
            ) : null}
              ...
              <div className="space-y-3">
                ...
                {catalogError ? (
                  <p className="text-destructive text-sm">
                    {t(
                      "pages.aiInterfaces.volcano.activation.catalogLoadFailed"
                    )}
                  </p>
                ) : null}
