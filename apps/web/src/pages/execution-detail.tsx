        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Execution details for each node in the workflow
              </CardDescription>
            </CardHeader>
            <CardContent>
              {execution.nodeExecutions && execution.nodeExecutions.length > 0 ? (
                <div className="space-y-4">
                  {execution.nodeExecutions.map((node, index) => (
                    <Card key={node.nodeId || index} className="border border-muted">
                      <CardHeader className="py-3">
                        <div className="flex justify-between items-center">
                          <CardTitle className="text-sm font-medium">{node.nodeName || `Node ${node.nodeId}`}</CardTitle>
                      ...
                      <CardContent className="py-3">
                        <Tabs defaultValue="overview">
                          <TabsList className="mb-2">
                            ...
                            {node.error && <TabsTrigger value="error">Error</TabsTrigger>}
                          </TabsList>
                          ...
                          <TabsContent value="overview">
                            <div className="space-y-1">
                              ...
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Completed:</span>
                                <span>{formatDateTime(node.endedAt)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Duration:</span>
                                <span>
                                  {node.startedAt ? calculateDuration(node.startedAt, node.endedAt) : "N/A"}
                                </span>
                              </div>
                            </div>
                          </TabsContent>
                          ...
                          {node.error && (
                            <TabsContent value="error">
                              <ScrollArea className="h-[200px] rounded-md border p-4 bg-destructive/10">
                ...
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No node executions found</p>
                </div>
              )}
            </CardContent>
          </Card>
