...
export class CloudflareBrowserLinksNode extends ExecutableNode {
  public static readonly nodeType: NodeType = {
    ...
    description: "Fetch all links from a rendered page using Cloudflare Browser Rendering.",
    category: "Net",
    icon: "link",
    inputs: [
      {
  ...
  async execute(context: NodeContext): Promise<NodeExecution> {
    ...
    try {
      ...
      if (!Array.isArray(json.result)) {
        return this.createErrorResult("Cloudflare API error: No links array returned");
      }
