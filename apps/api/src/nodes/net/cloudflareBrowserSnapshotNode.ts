...
export class CloudflareBrowserSnapshotNode extends ExecutableNode {
  public static readonly nodeType: NodeType = {
    ...
    description: "Get HTML content and screenshot from a rendered page using Cloudflare Browser Rendering.",
    category: "Net",
    icon: "camera",
    inputs: [
      {
  ...
  async execute(context: NodeContext): Promise<NodeExecution> {
    ...
    try {
      ...
      if (!json.result || typeof json.result.content !== "string" || !json.result.screenshot) {
        return this.createErrorResult("Cloudflare API error: Invalid snapshot response format");
      }

      // Convert base64 screenshot to buffer
      const screenshotData = Buffer.from(json.result.screenshot, 'base64');
      if (!screenshotData || screenshotData.length === 0) {
        return this.createErrorResult("Cloudflare API error: Invalid screenshot data");
      }
