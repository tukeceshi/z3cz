...
const createMockEnv = (masterKey?: string): Bindings => ({
  SECRET_MASTER_KEY: masterKey !== undefined ? masterKey : "a1b2c3d4e5f6789012345678901234567890abcdef1234567890abcdef123456",
...
describe("Encryption Utilities", () => {
  ...
  describe("Organization isolation", () => {
    ...
    it("should maintain organization isolation consistently", async () => {
      ...
      const org1Decrypted = await decryptSecret(org1Encrypted, mockEnv, "org-1");
      const org2Decrypted = await decryptSecret(org2Encrypted, mockEnv, "org-2");
  ...
  describe("Error handling - Invalid master key format", () => {
    it("should throw error for invalid hex characters", async () => {
      const envWithInvalidKey = createMockEnv("invalid-hex-characters-not-64-chars");
    ...
    it("should throw error for wrong key length (too short)", async () => {
      ...
      ).rejects.toThrow("SECRET_MASTER_KEY must be 64 hex characters (32 bytes)");
    });
    ...
    it("should throw error for non-hex characters", async () => {
      const envWithNonHex = createMockEnv("gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg"); // 64 'g' chars (invalid hex)
  ...
  describe("Error handling - Invalid encrypted data", () => {
    ...
    it("should throw error for empty encrypted data", async () => {
      await expect(
        decryptSecret("", mockEnv, testOrgId)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Key derivation consistency", () => {
    ...
    it("should produce different keys for different master keys", async () => {
      const env1 = createMockEnv("1111111111111111111111111111111111111111111111111111111111111111");
      const env2 = createMockEnv("2222222222222222222222222222222222222222222222222222222222222222");
