...
export function registerRequestToCatalogInsert(
  ...
  return {
    ...
    storageKey:
      resource.kind === "cloud" ? (resource.storageKey ?? null) : null,
...
export async function rekeyMediaResourceCatalogEntry(
  ...
  await rekeyMediaResource(db, {
    ...
    storageKey:
      params.request.kind === "cloud"
        ? (params.request.storageKey ?? null)
        : null,
  });
}
...
export async function resolveMediaResources(
  ...
  params: {
    ...
    /** When true, active character-library rows submit as asset:// instead of TOS. */
  ...
  for (const resourceId of trimmedIds) {
    ...
    if (
      ...
      resolved.push({
        ...
        cloudAccelerationStatus:
          catalogEntry.cloudAccelerationStatus ?? undefined,
    ...
    if (
      params.generationSubmit &&
      catalogEntry.source === CHARACTER_LIBRARY_SOURCE
    ) {
      const assetUrl = readCharacterLibrarySubmitUrl({
        source: catalogEntry.source,

        upstreamAssetId: catalogEntry.upstreamAssetId,

        upstreamAssetStatus: catalogEntry.upstreamAssetStatus,
      });
