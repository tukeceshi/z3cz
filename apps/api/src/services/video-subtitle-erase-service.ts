import {
  ...
  VIDEO_SUBTITLE_ERASE_JOB_KIND,
  VIDEO_SUBTITLE_ERASE_MODEL_CANONICAL_ID,
  VIDEO_SUBTITLE_ERASE_MODEL_DISPLAY_NAME,
...
import {
  pollMediaKitTask,
  submitMediaKitSubtitleEraseTask,
...
function buildVideoSubtitleErasePromptExcerpt(
  body: SubmitVideoSubtitleEraseRequest
): string {
  if (body.mode !== "refined") {
    return body.mode;
  }
  const parts: string[] = [
    body.mode,
    body.eraseMode,
    body.modelVersion,
    body.outputEncodeMode,
  ].filter((part) => typeof part === "string");
  if (body.eraseRatioLocation?.length) {
    parts.push(`${body.eraseRatioLocation.length} regions`);
  }
  return parts.join(" · ");
}
...
function assertSubtitleEraseModeEnabled(
  metadataRaw: string | null,
  mode: VolcanoMediaKitSubtitleEraseMode
): void {
  ...
  const enabledModes = listEnabledVolcanoMediaKitSubtitleEraseModes(mediaKit);
  if (!enabledModes.includes(mode)) {
    throw new Error(`Subtitle erase mode "${mode}" is not enabled`);
  }
}

export async function submitVideoSubtitleEraseTask(
  env: Bindings,
  params: {
    readonly organizationId: string;
    readonly userId?: string;
    readonly body: SubmitVideoSubtitleEraseRequest;
  }
): Promise<{
  ...
  assertSubtitleEraseModeEnabled(row.metadata, params.body.mode);
  ...
  const videoResourceIds = await registerGeneratingPlaceholderResources(db, {
    organizationId: params.organizationId,
    mimeType: "video/mp4",
    modelCanonicalId: VIDEO_SUBTITLE_ERASE_MODEL_CANONICAL_ID,
  });
  ...
  const job = await createGenerationJob(db, {
    ...
    resultJson: {
      jobKind: VIDEO_SUBTITLE_ERASE_JOB_KIND,
  ...
  await createAiModelInvocation(db, {
    ...
    promptExcerpt: buildVideoSubtitleErasePromptExcerpt(params.body),
    content: "",
    source: "ai-video-subtitle-erase-submit",
    status: "pending",
    workflowId: params.body.workflowId,
    nodeId: params.body.nodeId,
  ...
  let taskId: string;
  try {
    const submitResult = await submitMediaKitSubtitleEraseTask({
      apiKey,
      videoUrl: source.url,
      mode: params.body.mode,
      eraseMode: params.body.eraseMode,
      modelVersion: params.body.modelVersion,
      outputEncodeMode: params.body.outputEncodeMode,
      eraseRatioLocation: params.body.eraseRatioLocation,
...
export async function pollVideoSubtitleEraseGenerationJob(
  env: Bindings,
  db: Database,
  ...
  if (pollResult.status === "failed") {
    const failed = await updateGenerationJob(db, {
      ...
      failureReason: pollResult.error ?? "Subtitle erase failed",
    });
    if (failed) {
      await markMediaResourcesFailed(db, {
        organizationId: failed.organizationId,
