...
async function fetchUserInfo(
  ...
  try {
    ...
    console.log('User info fetched successfully:', JSON.stringify(userInfo).substring(0, 200));
    return userInfo;
  } catch (error) {
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    console.log(`Callback received - code: ${code?.substring(0, 10)}..., state: ${state}, provider: ${provider}`);
    
    // Validate required parameters
    if (!code || !state) {
      return new Response(JSON.stringify({ error: 'Missing required parameters: code and state are required' }), {
        status: 400,
    ...
    if (!provider) {
      return new Response(JSON.stringify({ error: 'Missing required parameter: provider is required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    // Exchange the authorization code for an access token
    const tokenResponse = await exchangeCodeForToken(code, redirectUri, provider, env);
    if (!tokenResponse) {
    ...
    console.log('Token exchange successful, access token:', tokenResponse.access_token?.substring(0, 10) + '...');
    
    // Fetch user information
    const userInfo = await fetchUserInfo(tokenResponse.access_token, provider, env);
    if (!userInfo) {
    ...
    const userEmail = config.userEmailField ? userInfo[config.userEmailField] : undefined;
    
    if (!userId || !userName) {
      console.error('Failed to extract user information:', JSON.stringify(userInfo));
