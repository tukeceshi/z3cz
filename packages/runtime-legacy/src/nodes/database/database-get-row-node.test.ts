...
import { DatabaseGetRowNode } from "./database-get-row-node";
...
describe("DatabaseGetRowNode", () => {
  const createNode = () =>
    new DatabaseGetRowNode({ nodeId: "database-get-row" } as unknown as Node);

  it("should return a matching row", async () => {
    const row = { id: 1, name: "Alice", email: "alice@example.com" };
    const connection = createMockConnection({ results: [row] });
    ...
    expect(result.outputs?.record).toEqual(row);
  ...
  it("should return null when row is not found", async () => {
    ...
    expect(result.status).toBe("completed");
    expect(result.outputs?.record).toBeNull();
  ...
  it("should work with string primary key", async () => {
    ...
    const row = { slug: "hello-world", title: "Hello World" };
    const connection = createMockConnection({ results: [row] });
    const node = createNode();
    const result = await node.execute(
      createContext(
