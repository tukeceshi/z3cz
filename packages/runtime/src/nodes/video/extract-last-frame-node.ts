...
/**
 * Extracts the last frame from a video as a JPEG image
 * using FFmpeg running in a Cloudflare Container.
 */
export class ExtractLastFrameNode extends MultiStepNode {
  ...
  public static readonly nodeType: NodeType = {
    ...
    description:
      "Extracts the last frame from a video as a JPEG image using FFmpeg in a Cloudflare Container",
    ...
    inputs: [
      {
        name: "video",
        type: "video",
        description: "Video to extract the last frame from",
        required: true,
      },
    ],
    outputs: [
      {
        name: "image",
        type: "image",
        description: "Last frame as JPEG",
      },
    ],
  };
  ...
  async execute(context: MultiStepNodeContext): Promise<NodeExecution> {
    ...
    try {
      const validatedInput = ExtractLastFrameNode.inputSchema.parse(
        context.inputs
      );
