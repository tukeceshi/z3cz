...
/**
 * X List User Posts node implementation
 * Retrieves posts by a specific user
 */
export class ListUserPostsXNode extends ExecutableNode {
  public static readonly nodeType: NodeType = {
    id: "list-user-posts-x",
    name: "List User Posts (X)",
    type: "list-user-posts-x",
    description: "Get posts by a specific user",
    tags: ["Social", "X", "User", "Posts", "List"],
    icon: "list",
    documentation:
  ...
  async execute(context: NodeContext): Promise<NodeExecution> {
    try {
      ...
      if (!response.ok) {
        ...
        return this.createErrorResult(
          `Failed to get user posts from X API: ${errorData}`
        );
      }
      ...
      return this.createErrorResult(
        error instanceof Error
          ? error.message
          : "Unknown error getting user posts from X"
      );
    }
  }
}
