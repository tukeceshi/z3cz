...
const SIZE_OPTIONS = ["2K", "4K", "custom"] as const;
...
/**
 * Seedream 4.5 node for generating images using ByteDance's model via Replicate.
 * @see https://replicate.com/bytedance/seedream-4.5
 */
export class Seedream45Node extends MultiStepNode {
  private static readonly inputSchema = z.object({
    ...
    width: z.number().int().min(1024).max(4096).optional().default(2048),
    height: z.number().int().min(1024).max(4096).optional().default(2048),
  ...
  public static readonly nodeType: NodeType = {
    id: "seedream-4-5",
    name: "Image Generation (Seedream 4.5)",
    type: "seedream-4-5",
    description:
      "Generates high-quality images from text prompts or reference images using ByteDance's Seedream 4.5 model via Replicate",
    tags: [
      "AI",
      "Image",
      "Replicate",
    ...
    icon: "image",
    documentation:
      "Generates images using ByteDance's Seedream 4.5 model via Replicate. Supports text-to-image and image-to-image generation with configurable resolution (2K/4K/custom), aspect ratio, and sequential generation mode. Use size='custom' with width and height to specify exact pixel dimensions (1024–4096). Note: 1K resolution is not supported.",
    referenceUrl: "https://replicate.com/bytedance/seedream-4.5",
    inlinable: false,
    usage: 100,
    inputs: [
      {
      ...
      },
      {
        ...
        type: "string",
        description:
          "Image resolution: 2K (2048px), 4K (4096px), or 'custom' for specific dimensions. Note: 1K is not supported.",
        value: "2K",
        hidden: true,
      },
      {
      ...
      },
      {
        ...
        type: "number",
        description:
          "Custom image width in pixels (only used when size='custom'). Range: 1024–4096.",
        value: 2048,
        hidden: true,
      },
      {
        ...
        type: "number",
        description:
          "Custom image height in pixels (only used when size='custom'). Range: 1024–4096.",
        value: 2048,
        hidden: true,
      },
      {
  ...
  async execute(context: MultiStepNodeContext): Promise<NodeExecution> {
    ...
    try {
      const validatedInput = Seedream45Node.inputSchema.parse(context.inputs);
      ...
      // aspect_ratio and width/height are mutually exclusive based on size
      ...
      if (result.status === "failed") {
        return this.createErrorResult(
          `Seedream 4.5 generation failed: ${result.error ?? "Unknown error"}`
        );
      }

      if (result.status === "canceled") {
        return this.createErrorResult("Seedream 4.5 generation was canceled");
      }

      if (result.status !== "succeeded") {
        return this.createErrorResult(
          "Seedream 4.5 generation timed out after 5 minutes"
        );
      }

      if (!result.output || result.output.length === 0) {
        return this.createErrorResult(
          "Seedream 4.5 generation succeeded but no output was returned"
        );
      }
