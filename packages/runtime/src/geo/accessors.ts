...
import { feature, featureCollection, multiLineString, multiPoint, multiPolygon, point } from "./helpers";
...
function coordEach(
  ...
  function processCoordinates(coords: Coordinate | Coordinate[] | Coordinate[][] | Coordinate[][][]): void {
  ...
  function processFeature(feat: Feature): void {
    ...
    if (geom.type === "GeometryCollection") {
      for (const g of (geom as unknown as GeometryCollection).geometries) {
        processFeature({ type: "Feature", geometry: g, properties: feat.properties });
      }
      return;
    }
    ...
    switch (geom.type) {
      case "MultiPoint":
        for (const coord of (geom as MultiPoint).coordinates) {
          result.push(feature({ type: "Point", coordinates: coord } as Point, { ...feat.properties }));
        }
        break;
      case "MultiLineString":
        for (const coord of (geom as MultiLineString).coordinates) {
          result.push(feature({ type: "LineString", coordinates: coord } as LineString, { ...feat.properties }));
        }
        break;
      case "MultiPolygon":
        for (const coord of (geom as MultiPolygon).coordinates) {
          result.push(feature({ type: "Polygon", coordinates: coord } as Polygon, { ...feat.properties }));
        }
        break;
      default:
        result.push(feat);
...
export function flip(geojson: AnyGeoJSON): AnyGeoJSON {
  function flipCoord(coord: Coordinate): Coordinate {
    return coord.length === 3 ? [coord[1], coord[0], coord[2]] : [coord[1], coord[0]];
  }
  ...
  function flipGeometry(geom: Geometry | GeometryCollection): Geometry | GeometryCollection {
    if ("geometries" in geom) {
      return {
    ...
    return {
      ...geom,
      coordinates: flipCoords((geom as Geometry).coordinates, coordDepth(geom.type)),
    } as Geometry;
  }
