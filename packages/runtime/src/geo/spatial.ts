...
export function nearestPoint(
...
): Feature<Point> & { properties: { featureIndex: number; distanceToPoint: number } } {
  ...
  return result as Feature<Point> & { properties: { featureIndex: number; distanceToPoint: number } };
}
...
export function nearestPointOnLine(
  ...
  options?: { units?: Units },
): Feature<Point> & { properties: { index: number; dist: number; location: number; multiFeatureIndex: number } } {
  ...
  function processLine(coords: Coordinate[], multiIndex: number): void {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      if (d < minDist) {
        ...
        bestIndex = i;
        bestLocation = location + distance(coords[i] as number[], proj.point, options);
        bestMultiIndex = multiIndex;
      }
      location += distance(coords[i] as number[], coords[i + 1] as number[], options);
    }
  }

  if (lines.type === "FeatureCollection") {
    for (let mi = 0; mi < (lines as FeatureCollection<LineString>).features.length; mi++) {
      processLine((lines as FeatureCollection<LineString>).features[mi].geometry.coordinates as Coordinate[], mi);
    }
  } else {
    const geom = lines.type === "Feature" ? (lines as Feature<LineString>).geometry : lines as LineString;
  ...
  }) as Feature<Point> & { properties: { index: number; dist: number; location: number; multiFeatureIndex: number } };
}
...
export function pointOnFeature(geojson: AnyGeoJSON): Feature<Point> {
  ...
  if (geojson.type === "Feature") {
    ...
    if (geom.type === "Polygon") {
      if (booleanPointInPolygon([centerX, centerY], geojson as Feature<Polygon>)) {
  ...
  if (geojson.type === "Feature" && (geojson as Feature).geometry.type === "Point") {
...
export function pointToLineDistance(
  ...
  const coord = getCoord(pt);
  const geom = line.type === "Feature" ? (line as Feature<LineString>).geometry : line as LineString;
...
export function pointToPolygonDistance(
  ...
  const coord = getCoord(pt);
  const geom = poly.type === "Feature" ? (poly as Feature<Polygon>).geometry : poly as Polygon;
...
export function polygonTangents(
  ...
  return featureCollection([point(tangentMin as number[]), point(tangentMax as number[])]);
}
...
export function centerMedian(
  ...
  for (const feat of features.features) {
    ...
    const w = options?.weight && feat.properties
...
export function polygonSmooth(
  ...
  if (geojson.type === "FeatureCollection") {
    ...
    features.push(smoothFeature({ type: "Feature", properties: {}, geometry: geojson as Polygon }));
  }
...
function chaikinSmooth(ring: Coordinate[]): Coordinate[] {
  const isRing = ring[0][0] === ring[ring.length - 1][0] && ring[0][1] === ring[ring.length - 1][1];
