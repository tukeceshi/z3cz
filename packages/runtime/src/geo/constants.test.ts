...
describe("radiansToDegrees", () => {
  ...
  it("converts PI/2 radians", () => {
    ...
    expect(radiansToDegrees(Math.PI / 2)).toBe(turfRadiansToDegrees(Math.PI / 2));
  });
...
describe("lengthToRadians and radiansToLength roundtrip", () => {
  ...
  for (const unit of units) {
    ...
    it(`matches turf for lengthToRadians with ${unit}`, () => {
      const distance = 42.5;
      expect(lengthToRadians(distance, unit)).toBe(turfLengthToRadians(distance, unit));
    });

    it(`matches turf for radiansToLength with ${unit}`, () => {
      const radians = 0.005;
      expect(radiansToLength(radians, unit)).toBe(turfRadiansToLength(radians, unit));
    });
  }
