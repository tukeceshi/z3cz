...
describe("polygon", () => {
  ...
  it("throws on unclosed ring", () => {
    ...
    expect(() => polygon([unclosed])).toThrow("First and last Position are not equivalent.");
  });
...
describe("getGeom", () => {
  ...
  it("returns geometry collection from a Feature", () => {
    const gc = geometryCollection([{ type: "Point" as const, coordinates: [0, 0] }]);
    const turfGc = turf.geometryCollection([{ type: "Point" as const, coordinates: [0, 0] }]);
