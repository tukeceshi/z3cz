...
export function bboxPolygon(bb: BBox, options?: { properties?: Record<string, unknown>; id?: string | number }): Feature<Polygon> {
  ...
  return polygon([ring], options?.properties, options?.id !== undefined ? { id: options.id } : undefined);
}

export function envelope(geojson: Feature | FeatureCollection): Feature<Polygon> {
...
export function circle(
  ...
  radius: number,
  options?: { steps?: number; units?: Units; properties?: Record<string, unknown> },
): Feature<Polygon> {
...
export function sector(
  ...
  const arc = lineArc(center, radius, bearing1, bearing2, { steps, units: options?.units });
  const arcCoords = arc.geometry.coordinates as Coordinate[];

  const coords: Coordinate[] = [centerCoord as Coordinate, ...arcCoords, centerCoord as Coordinate];
  return polygon([coords], options?.properties);
}
...
export function destination(
  ...
  return point([radiansToDegrees(lng2), radiansToDegrees(lat2)], options?.properties);
}
...
export function rhumbDestination(
  ...
  return point([radiansToDegrees(lambda2), radiansToDegrees(phi2)], options?.properties);
}
...
export function along(
  ...
  const coords = line.type === "Feature"
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    ...
    if (travelled + segDist >= dist) {
      ...
      const direction = bearing(coords[i] as number[], coords[i + 1] as number[]);
...
export function greatCircle(
  ...
  options?: { properties?: Record<string, unknown>; npoints?: number; offset?: number },
): Feature<LineString> {
...
function createGreatCircleInterpolator(from: number[], to: number[]): (f: number) => Coordinate {
...
function bufferFeature(
  ...
  if (geom.type === "Point") {
    return circle(geom as Point, Math.abs(radius), { units, steps, properties: feat.properties ?? {} });
  }

  if (geom.type === "LineString") {
    return bufferLineString((geom as LineString).coordinates as Coordinate[], radius, units, steps, feat.properties);
  }

  if (geom.type === "Polygon") {
    return bufferPolygon((geom as Polygon).coordinates as Coordinate[][], radius, units, steps, feat.properties);
  }
...
function bufferLineString(
  ...
  // Create start cap
  const startCap = createCap(coords[0], coords[1], radius, units, steps, "start");
  // Create end cap
  ...
  const ring: Coordinate[] = [...left, ...endCap, ...right.reverse(), ...startCap];
...
function bufferRing(ring: Coordinate[], radius: number, units: Units, steps: number): Coordinate[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const dest = destinationCalc(curr, Math.abs(radius), radius > 0 ? offsetBearing : offsetBearing + 180, units);
    result.push(dest);
...
function offsetCoords(
  ...
  for (let i = 0; i < coords.length; i++) {
    const bear = i < coords.length - 1
      ? bearing(coords[i], coords[i + 1])
      : bearing(coords[i - 1], coords[i]);

    const offsetBear = side === "left" ? bear - 90 : bear + 90;
    result.push(destinationCalc(coords[i], Math.abs(radius), offsetBear, units));
  }
  return result;
}
...
function destinationCalc(origin: Coordinate | number[], dist: number, bear: number, units: Units): Coordinate {
