...
function getCoords(geom: Geometry): Coordinate | Coordinate[] | Coordinate[][] | Coordinate[][][] {
  return geom.coordinates;
}

export function booleanClockwise(line: Feature<LineString> | LineString | Coordinate[]): boolean {
  let ring: Coordinate[];
  if (Array.isArray(line) && Array.isArray(line[0])) {
    ring = line as Coordinate[];
  } else {
    const geom = (line as Feature<LineString>).type === "Feature"
...
export function booleanContains(
  ...
  if (geom2.type === "Point") {
    ...
    if (geom1.type === "Polygon") {
      return pointInPolygon(pt, (geom1 as Polygon).coordinates as Coordinate[][]);
    }
    ...
    if (geom1.type === "Point") {
      return pt[0] === (geom1 as Point).coordinates[0] && pt[1] === (geom1 as Point).coordinates[1];
    }
  }
  ...
  if (geom2.type === "Polygon" && geom1.type === "Polygon") {
    ...
    for (const coord of ring2) {
      if (!pointInPolygon(coord, (geom1 as Polygon).coordinates as Coordinate[][])) {
        return false;
      }
    }
    return true;
...
export function booleanCrosses(
  ...
  if (geom1.type === "MultiPoint" && geom2.type === "Polygon") {
    ...
    for (const coord of (geom1 as MultiPoint).coordinates) {
      if (pointInPolygon(coord, (geom2 as Polygon).coordinates as Coordinate[][])) {
        inside = true;
      } else {
        outside = true;
      }
...
export function booleanEqual(
  ...
  return JSON.stringify(sortCoords(geom1)) === JSON.stringify(sortCoords(geom2));
}

function sortCoords(geom: Geometry | GeometryCollection): Geometry | GeometryCollection {
...
export function booleanIntersects(
  ...
  if (geom1.type === "Point" && geom2.type === "Point") {
    return (geom1 as Point).coordinates[0] === (geom2 as Point).coordinates[0] &&
           (geom1 as Point).coordinates[1] === (geom2 as Point).coordinates[1];
  }

  // Point vs Polygon
  if (geom1.type === "Point" && (geom2.type === "Polygon" || geom2.type === "MultiPolygon")) {
    return booleanPointInPolygon(geom1 as Point, feature2 as Feature<Polygon | MultiPolygon>);
  }
  if ((geom1.type === "Polygon" || geom1.type === "MultiPolygon") && geom2.type === "Point") {
    return booleanPointInPolygon(geom2 as Point, feature1 as Feature<Polygon | MultiPolygon>);
  }
...
export function booleanOverlap(
  ...
  if (geom1.type === "LineString" && geom2.type === "LineString") {
    ...
    return booleanIntersects(feature1, feature2) && !booleanWithin(feature1, feature2) && !booleanWithin(feature2, feature1);
  }

  // Polygons overlap if they intersect but neither contains the other
  return !booleanContains(feature1, feature2) && !booleanContains(feature2, feature1);
}
...
export function booleanPointInPolygon(
  ...
  if (geom.type === "Polygon") {
    return pointInPolygon(coord as Coordinate, (geom as Polygon).coordinates as Coordinate[][], ignoreBoundary);
  }

  if (geom.type === "MultiPolygon") {
    for (const polyCoords of (geom as MultiPolygon).coordinates) {
      if (pointInPolygon(coord as Coordinate, polyCoords as Coordinate[][], ignoreBoundary)) {
        return true;
      }
    }
  }
...
export function booleanPointOnLine(
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    if (options?.ignoreEndVertices) {
      ...
      if (i === coords.length - 2 && coordsNearlyEqual(coord, coords[coords.length - 1], epsilon)) continue;
    }

    if (isPointOnSegment(coord as Coordinate, coords[i], coords[i + 1], epsilon)) {
      return true;
    }
  }
...
export function booleanTouches(
  ...
  if (geom1.type === "Point" && geom2.type === "Polygon") {
    ...
    return isPointOnBoundary(coord, (geom2 as Polygon).coordinates as Coordinate[][]) &&
           !pointInPolygon(coord, (geom2 as Polygon).coordinates as Coordinate[][], true);
  }

  if (geom1.type === "Polygon" && geom2.type === "Point") {
    const coord = (geom2 as Point).coordinates;
    return isPointOnBoundary(coord, (geom1 as Polygon).coordinates as Coordinate[][]) &&
           !pointInPolygon(coord, (geom1 as Polygon).coordinates as Coordinate[][], true);
  }
  ...
  if (geom1.type === "LineString" && geom2.type === "Polygon") {
    ...
    const endOnBoundary = isPointOnBoundary(coords[coords.length - 1], polyCoords);
  ...
  if (geom1.type === "Point" && geom2.type === "LineString") {
    ...
    return coordsNearlyEqual(coord, lineCoords[0]) || coordsNearlyEqual(coord, lineCoords[lineCoords.length - 1]);
  }
  ...
  switch (geom.type) {
    ...
    case "MultiPoint": {
      return (geom as MultiPoint).coordinates.every(
        (c) => c.length >= 2 && c.every((v) => typeof v === "number" && isFinite(v)),
      );
    }
    case "LineString": {
    ...
    case "MultiLineString": {
      return (geom as MultiLineString).coordinates.every((line) => line.length >= 2);
    }
    case "Polygon": {
    ...
    case "GeometryCollection": {
      return (geom as GeometryCollection).geometries.every((g) => booleanValid(g));
    }
    default:
      return false;
  }
...
function pointInPolygon(
  ...
  for (let i = 0, j = outerRing.length - 1; i < outerRing.length; j = i++) {
    ...
    if (!ignoreBoundary) {
      if (isPointOnSegment(coord as Coordinate, outerRing[i], outerRing[j])) {
        return true;
      }
    }
...
function isPointOnSegment(p: Coordinate, a: Coordinate, b: Coordinate, epsilon = 0): boolean {
...
function coordsNearlyEqual(a: Coordinate | number[], b: Coordinate | number[], epsilon = 0): boolean {
...
function lineIntersectsPolygon(lineCoords: Coordinate[], polyCoords: Coordinate[][]): boolean {
  ...
  for (const ring of polyCoords) {
    for (let i = 0; i < ring.length - 1; i++) {
      for (let j = 0; j < lineCoords.length - 1; j++) {
        if (segmentsIntersect(ring[i], ring[i + 1], lineCoords[j], lineCoords[j + 1])) {
          return true;
        }
      }
    }
...
function polygonsIntersect(coords1: Coordinate[][], coords2: Coordinate[][]): boolean {
...
function geomBBox(geom: Geometry | GeometryCollection): [number, number, number, number] {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
...
function bboxOverlap(bb1: [number, number, number, number], bb2: [number, number, number, number]): boolean {
