...
function getPolygonGeom(feat: Feature<Polygon | MultiPolygon> | Polygon | MultiPolygon): Polygon | MultiPolygon {
  if (feat.type === "Feature") return (feat as Feature<Polygon | MultiPolygon>).geometry;
...
function fromPolyclipResult(
  ...
  if (multiCoords.length === 1) {
    return {
      ...
      geometry: { type: "Polygon", coordinates: multiCoords[0] as Coordinate[][] },
    } as Feature<Polygon>;
  }
  ...
  return {
    type: "Feature",
    properties: properties ?? {},
    geometry: { type: "MultiPolygon", coordinates: multiCoords as Coordinate[][][] },
...
export function intersect(
  ...
  return fromPolyclipResult(result as PolyclipGeom, options?.properties ?? poly1.properties);
}
...
export function bboxClip(
  ...
  const clipped = fromPolyclipResult(result as PolyclipGeom, feature.properties);
  if (!clipped) {
