...
describe("lineSlice", () => {
  it("should slice a line between two points, matching turf", () => {
    ...
    for (let i = 0; i < ours.geometry.coordinates.length; i++) {
      expect(ours.geometry.coordinates[i][0]).toBeCloseTo(
        theirs.geometry.coordinates[i][0],
        4
      );
      expect(ours.geometry.coordinates[i][1]).toBeCloseTo(
        theirs.geometry.coordinates[i][1],
        4
      );
    }
  });
...
describe("lineSliceAlong", () => {
  it("should slice a line by distance in km, matching turf", () => {
    ...
    const ours = lineSliceAlong(line, startDist, stopDist, {
      units: "kilometers",
    });
    const theirs = turf.lineSliceAlong(line, startDist, stopDist, {
      units: "kilometers",
    });
...
describe("lineOffset", () => {
  it("should offset a line left and right, comparing with turf", () => {
    ...
    const theirsLeft = turf.lineOffset(line, offsetDist, {
      units: "kilometers",
    });

    for (let i = 0; i < oursLeft.geometry.coordinates.length; i++) {
      expect(oursLeft.geometry.coordinates[i][0]).toBeCloseTo(
        theirsLeft.geometry.coordinates[i][0],
        4
      );
      expect(oursLeft.geometry.coordinates[i][1]).toBeCloseTo(
        theirsLeft.geometry.coordinates[i][1],
        4
      );
    }

    const oursRight = lineOffset(line, -offsetDist, { units: "kilometers" });
    const theirsRight = turf.lineOffset(line, -offsetDist, {
      units: "kilometers",
    });

    for (let i = 0; i < oursRight.geometry.coordinates.length; i++) {
      expect(oursRight.geometry.coordinates[i][0]).toBeCloseTo(
        theirsRight.geometry.coordinates[i][0],
        4
      );
      expect(oursRight.geometry.coordinates[i][1]).toBeCloseTo(
        theirsRight.geometry.coordinates[i][1],
        4
      );
    }
  });
...
describe("lineSplit", () => {
  it("should split a line at a point into two parts", () => {
    ...
    const firstEnd =
      ours.features[0].geometry.coordinates[
        ours.features[0].geometry.coordinates.length - 1
      ];
...
describe("lineIntersect", () => {
  it("should find intersection points for two crossing lines (X shape)", () => {
    ...
    expect(ours.features[0].geometry.coordinates[0]).toBeCloseTo(
      theirs.features[0].geometry.coordinates[0],
      4
    );
    expect(ours.features[0].geometry.coordinates[1]).toBeCloseTo(
      theirs.features[0].geometry.coordinates[1],
      4
    );
  });
...
describe("polygonToLine", () => {
  it("should convert a Polygon to a LineString", () => {
    ...
    const ourCoords = ((ours as GeoJSON.Feature).geometry as GeoJSON.LineString)
      .coordinates;
    const theirCoords = (
      (theirs as GeoJSON.Feature).geometry as GeoJSON.LineString
    ).coordinates;
    expect(ourCoords.length).toBe(theirCoords.length);
    for (let i = 0; i < ourCoords.length; i++) {
      expect(ourCoords[i][0]).toBeCloseTo(theirCoords[i][0], 4);
  ...
  it("should convert a Polygon with holes to multiple LineStrings", () => {
    ...
    // Both should produce a Feature with MultiLineString
    expect(ours.type).toBe(theirs.type);
    expect((ours as GeoJSON.Feature).geometry.type).toBe(
      (theirs as GeoJSON.Feature).geometry.type
    );
  });
});
