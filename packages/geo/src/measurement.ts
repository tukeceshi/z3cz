...
export function rhumbDistance(
  ...
  const c2 = getCoord(to);
  const _destPoint =
    options?.units === "degrees" ? [c2[0], c2[1]] : [c2[0], c2[1]];
...
export function length(
  ...
  return segmentReduce(
    ...
    (total, segment) => {
      ...
      return (
        total + distance(coords[0] as number[], coords[1] as number[], options)
      );
    },
    0
  );
...
function calculateArea(geom: Geometry | GeometryCollection): number {
  ...
  if (geom.type === "MultiPolygon") {
    let total = 0;
    for (const poly of (geom as unknown as { coordinates: Coordinate[][][] })
      .coordinates) {
...
export function centroid(
  ...
  let ySum = 0;
  let len = 0;
  // Use excludeWrapCoord=true to match turf: skip closing coord of polygon rings
  coordEachExcludeWrap(geojson, (coord) => {
    xSum += coord[0];
    ySum += coord[1];
    len++;
  });
...
function coordEachExcludeWrap(
  geojson: AnyGeoJSON,
  callback: (coord: Coordinate) => void
): void {
  function processRing(ring: Coordinate[], excludeLast: boolean): void {
    const end = excludeLast ? ring.length - 1 : ring.length;
    for (let i = 0; i < end; i++) {
      callback(ring[i]);
    }
  }
  ...
  function processGeom(geom: Geometry | GeometryCollection): void {
    ...
    switch (geom.type) {
      case "Point":
        callback(geom.coordinates as Coordinate);
        break;
      case "MultiPoint":
      case "LineString":
        for (const c of geom.coordinates as Coordinate[]) callback(c);
        break;
      case "MultiLineString":
        for (const line of geom.coordinates as Coordinate[][]) {
          for (const c of line) callback(c);
        }
        break;
      case "Polygon":
        for (const ring of geom.coordinates as Coordinate[][])
          processRing(ring, true);
        break;
      case "MultiPolygon":
        for (const poly of geom.coordinates as Coordinate[][][]) {
          for (const ring of poly) processRing(ring, true);
        }
        break;
    }
  }
...
export function centerOfMass(
  ...
  if (type === "Polygon") {
    ...
    const geom = getGeometry(geojson) as unknown as {
