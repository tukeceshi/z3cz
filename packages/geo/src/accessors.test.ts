...
import { bbox, combine, explode, flatten, flip } from "./accessors";
...
describe("combine", () => {
  it("should match turf geometry for a mix of Points, LineStrings, and Polygons", () => {
    ...
    const oursByType = Object.fromEntries(
      ours.features.map((f) => [f.geometry.type, f.geometry])
    );
    const turfsByType = Object.fromEntries(
      turfs.features.map((f) => [f.geometry.type, f.geometry])
    );
    expect(Object.keys(oursByType).sort()).toEqual(
      Object.keys(turfsByType).sort()
    );
