...
export function lineSegment(
  ...
  if (geojson.type === "FeatureCollection") {
    ...
    processGeom(
      geojson as LineString | MultiLineString | Polygon | MultiPolygon
    );
  }
...
export function lineToPolygon(
  ...
  if (line.type === "FeatureCollection") {
    ...
    return {
      ...
      geometry: {
        type: "MultiPolygon",
        coordinates: rings.map((r) => [r]) as Coordinate[][][],
      },
  ...
  if (geom.type === "MultiLineString") {
    ...
    // Treat as a single Polygon: first ring = outer, rest = holes
    return polygon(rings, options?.properties);
  }
...
export function polygonToLine(
...
): Feature<LineString | MultiLineString> | FeatureCollection<LineString> {
  const geom = poly.type === "Feature" ? (poly as Feature).geometry : poly;
  const props =
    options?.properties ??
    (poly.type === "Feature" ? (poly as Feature).properties : {});
  ...
  if (geom.type === "Polygon") {
    ...
    // Multiple rings → Feature<MultiLineString>
    return {
      type: "Feature",
      properties: props ?? {},
  ...
  if (allRings.length === 1) return lineString(allRings[0], props ?? {});
  return featureCollection(
    allRings.map((ring) => lineString(ring, props ?? {}))
  );
}
...
function getLineCoords(line: Feature<LineString> | LineString): Coordinate[] {
  if (line.type === "Feature")
    return (line as Feature<LineString>).geometry.coordinates as Coordinate[];
