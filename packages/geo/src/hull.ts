...
import { getCoord, polygon } from "./helpers";
...
export function concave(
  ...
  for (let i = 0; i < triIndices.length; i += 3) {
    triangles.push([triIndices[i], triIndices[i + 1], triIndices[i + 2]]);
  }
...
function grahamScan(points: Coordinate[]): Coordinate[] {
  ...
  for (let i = 2; i < sorted.length; i++) {
    while (
      stack.length > 1 &&
      cross(stack[stack.length - 2], stack[stack.length - 1], sorted[i]) <= 0
    ) {
      stack.pop();
    }
    stack.push(sorted[i]);
