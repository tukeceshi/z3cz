...
import { centroid, rhumbBearing, rhumbDistance } from "./measurement";
import type {
  Coordinate,
  Feature,
  FeatureCollection,
...
export function transformScale(
  ...
  coordEach(result, (coord) => {
    const dist = rhumbDistance(originCoord, coord);
    const bear = rhumbBearing(originCoord, coord);
    const newPoint = rhumbDestinationRaw(originCoord, dist * factor, bear);
    coord[0] = newPoint[0];
    coord[1] = newPoint[1];
  });
...
export function transformRotate(
  ...
  const pivot = options?.pivot
    ? getCoord(options.pivot as Feature<Point> | number[])
    : getCoord(centroid(geojson));
  const result = options?.mutate
    ? geojson
    : JSON.parse(JSON.stringify(geojson));
...
export function transformTranslate(
  ...
  if (dist === 0 && (!options?.zTranslation || options.zTranslation === 0))
    return geojson;
...
export function rewind(
  ...
  if (result.type === "FeatureCollection") {
    for (const f of (result as FeatureCollection).features)
...
export function cleanCoords(
  ...
  function cleanRing(coords: Coordinate[]): Coordinate[] {
    ...
    const deduped: Coordinate[] = [coords[0]];
    for (let i = 1; i < coords.length; i++) {
      if (
    ...
    // Remove collinear points (points that lie exactly on the line between prev and next)
    if (deduped.length <= 2) return deduped;
    const cleaned: Coordinate[] = [deduped[0]];
...
export function simplify(
  ...
  function simplifyRing(coords: Coordinate[], isRing: boolean): Coordinate[] {
    ...
    // Match turf/simplify-js: always run radial-dist first, then DP
    // When highQuality is true, only run DP (skip radial-dist pre-pass)
    const simplified = highQuality
      ? douglasPeucker(coords, tolerance)
      : douglasPeucker(simplifyRadialDist(coords, tolerance), tolerance);
  ...
  if (result.type === "FeatureCollection") {
    for (const f of (result as FeatureCollection).features)
      processGeomOrCollection(f.geometry);
  } else if (result.type === "Feature") {
