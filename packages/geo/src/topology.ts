  ...
  function getSegments(): [Coordinate, Coordinate][] {
    ...
    switch (geom.type) {
      ...
      case "MultiLineString":
        ...
        break;
      case "Polygon":
        for (const ring of (geom as Polygon).coordinates as Coordinate[][])
          addLine(ring);
        break;
    }
    return segs;
...
export function polygonize(
  ...
  for (const feat of lines.features) {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      if (!nodeMap.has(aKey))
        nodeMap.set(aKey, { coord: coords[i], neighbors: [] });
      if (!nodeMap.has(bKey))
        nodeMap.set(bKey, { coord: coords[i + 1], neighbors: [] });
...
function findMinimalCycle(
  ...
  while (steps++ < maxSteps) {
    ...
    if (bestNext === startKey) {
      ...
      const ring: Coordinate[] = path
        .map((k) => nodeMap.get(k)?.coord)
