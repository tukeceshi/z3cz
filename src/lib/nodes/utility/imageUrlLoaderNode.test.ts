...
describe('ImageUrlLoaderNode', () => {
  const mockNode: Node = {
    ...
    position: { x: 0, y: 0 },
    inputs: [
      { name: 'url', type: 'string', description: 'The URL of the PNG image to load' }
    ],
    outputs: [
      { name: 'imageData', type: 'binary', description: 'The image data as a binary array' }
    ]
  };
  ...
  it('should return error if content type is not an image', async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toBe('URL does not point to an image (content-type: text/html)');
  });
