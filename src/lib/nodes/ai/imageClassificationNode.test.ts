...
describe('ImageClassificationNode', () => {
  const mockNode: Node = {
    ...
    position: { x: 0, y: 0 },
    inputs: [
      { name: 'image', type: 'binary', description: 'The image to use for object detection' }
    ],
    outputs: [
      { name: 'detections', type: 'array', description: 'Array of detected objects with scores, labels, and bounding boxes' }
    ]
  };
