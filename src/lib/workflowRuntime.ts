...
export class WorkflowRuntime {
  ...
  private getNodeInputs(nodeId: string): Record<string, any> {
    ...
    for (const edge of incomingEdges) {
      ...
      if (sourceNodeOutputs && sourceNodeOutputs[edge.sourceOutput] !== undefined) {
  ...
  private async executeNode(nodeId: string): Promise<void> {
    ...
    if (!executableNode) {
      ...
      if (this.options.onNodeError) {
        this.options.onNodeError(nodeId, `Node type not implemented: ${nodeType}`);
      }
      return;
    }
  ...
  async execute(): Promise<Map<string, Record<string, any>>> {
    try {
      ...
      const startNodes = this.workflow.nodes.filter(node => 
        !this.workflow.edges.some(edge => edge.target === node.id)
      );
      ...
      const unexecutedNodes = this.workflow.nodes.filter(
        node => !this.executedNodes.has(node.id) && !this.nodeErrors.has(node.id)
      );
      ...
      if (this.options.onExecutionError) {
        this.options.onExecutionError(error instanceof Error ? error.message : 'Unknown error');
      }
