...
export class JsonNumberExtractorNode extends BaseExecutableNode {
  public static readonly nodeType: NodeType = {
    ...
    inputs: [
      ...
      },
      {
        name: 'path',
        type: 'string',
        description: 'The JSONPath expression (e.g., "$.user.profile.age" or "$.product.price")',
      },
      {
        name: 'defaultValue',
        type: 'number',
  ...
  public async execute(context: NodeContext): Promise<ExecutionResult> {
    try {
      ...
      const defaultValue = typeof context.inputs['defaultValue'] === 'number' ? context.inputs['defaultValue'] : 0;
      ...
      try {
        ...
        const numberValue = results.find(value => typeof value === 'number' && !isNaN(value));
      ...
      const error = err as Error;
      return this.createErrorResult(`Error extracting number: ${error.message}`);
    }
  }
} 