...
export class JsonBooleanExtractorNode extends BaseExecutableNode {
  public static readonly nodeType: NodeType = {
    ...
    inputs: [
      ...
      },
      {
        name: 'path',
        type: 'string',
        description: 'The JSONPath expression (e.g., "$.user.profile.isActive" or "$.settings.enabled")',
      },
      {
        name: 'defaultValue',
        type: 'boolean',
  ...
  public async execute(context: NodeContext): Promise<ExecutionResult> {
    try {
      ...
      const defaultValue = typeof context.inputs['defaultValue'] === 'boolean' ? context.inputs['defaultValue'] : false;
      ...
      try {
        ...
        const booleanValue = results.find(value => typeof value === 'boolean');
      ...
      const error = err as Error;
      return this.createErrorResult(`Error extracting boolean: ${error.message}`);
    }
  }
} 