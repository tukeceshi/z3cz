...
export class UpdateManager {
  ...
  constructor(options = {}) {
    ...
    this.compose =
      options.compose ||
      createComposeRunner(this.hostDir, this.composePath, this.envPath);
    this.sleep =
      options.sleep ||
  ...
  async runUpdate(fromVersion, targetVersion) {
    try {
      ...
      await runCommand(this.hostDir, "docker", ["pull", `${apiImageName}:${tag}`]);
      await runCommand(this.hostDir, "docker", ["pull", `${appImageName}:${tag}`]);
      ...
      await runCommand(this.hostDir, path.join(this.hostDir, "launcher"), [
        "migrate",
      ]);
      ...
      const backup = this.state.lastBackup;
      if (backup && this.state.rollbackVersion) {
        await this.runRollback(fromVersion, backup, true, error);
        return;
      }
