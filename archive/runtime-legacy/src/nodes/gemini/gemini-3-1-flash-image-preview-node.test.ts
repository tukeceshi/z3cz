...
import { Gemini31FlashImagePreviewNode } from "./gemini-3-1-flash-image-preview-node";

describe("Gemini31FlashImagePreviewNode", () => {
  ...
  const nodeId = "gemini-3-1-flash-image-preview";
  const node = new Gemini31FlashImagePreviewNode({
    nodeId,
  } as unknown as Node);
  ...
  describe("execute", () => {
    ...
    it("should handle extended aspect ratios", async () => {
      const result = await node.execute(
        createContext({
          prompt: "A tall vertical image",
          aspectRatio: "1:8",
        })
      );
    ...
    it("should handle 512px image size", async () => {
      const result = await node.execute(
        createContext({
          prompt: "A small thumbnail",
          imageSize: "512px",
        })
      );
