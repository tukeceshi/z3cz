...
export const PACKAGED_API_IMAGE = "tukeceshi/z3cz-api:latest";
export const PACKAGED_APP_IMAGE = "tukeceshi/z3cz-app:latest";

function readPackagedImages() {
  ...
  for (const raw of fs.readFileSync(file, "utf8").split("\n")) {
    ...
    if (line.startsWith("API_IMAGE=")) {
      api = line.slice("API_IMAGE=".length).trim() || api;
    }
    if (line.startsWith("APP_IMAGE=")) {
      app = line.slice("APP_IMAGE=".length).trim() || app;
    }
  }
  return { api, app };
}
...
export function renderCompose(config) {
  ...
  const images = readPackagedImages();
